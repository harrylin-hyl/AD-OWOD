...
class GroundingDinoTransformerDecoderLayer(
    ...
    def _init_layers(self) -> None:
        ...
        self.adapters = ModuleList(adapter_list)
    ...
    def forward(self,
        ...
        query = self.self_attn(
            ...
            **kwargs)
        query = query + self.adapters[0](query) # adapter
        ...
        query = query + self.adapters[1](query) # adapter
        ...
        query = self.cross_attn(
            ...
            **kwargs)
        query = query + self.adapters[2](query) # adapter
        query = self.norms[2](query)
        query = self.ffn(query)
        query = query + self.adapters[3](query) # adapter
...
class AdapterGroundingDinoTransformerEncoder(DeformableDetrTransformerEncoder):
    ...
    def _init_layers(self) -> None:
        ...
        self.image_adapters = ModuleList(image_adapter_list)
        ...
        self.text_adapters = ModuleList(text_adapter_list)
    ...
    def forward(self,
        ...
        for layer_id, layer in enumerate(self.layers):
            if self.fusion_layers:
                ...
                # adapter
                output = output + self.image_adapters[layer_id](output)
                memory_text = memory_text + self.text_adapters[layer_id](memory_text)
            if self.text_layers:
...
class AdapterGroundingDinoTransformerDecoder(DinoTransformerDecoder):
