...
class DetInferencer(BaseInferencer):
    ...
    def __init__(self,
                 ...
                 palette: str = 'none',
                 save_json_path = 'groundingdino_instances_results.json',
    ...
    def __call__(
        ...
        for ori_imgs, data in (track(inputs, description='Inference')
            ...
            img_id = int(ori_imgs[0].split("/")[-1].split(".")[0])
            classes = results['predictions'][0]["labels"]
            boxes = results['predictions'][0]["bboxes"]
            ...
            for k in range(len(classes)):
                category_id = classes[k] + 1 # change from 0,1,2, ... to 1,2,3, ...
                # result = {
                #     "image_id": img_id,
        ...
        # with open(osp.join('json_output_dir', '{}_instances_results.json'.format("grounding_dino-small-text_dual_encoder_decoder_lr1e-4_grad_clip0.1_text_cos_loss")), 'w') as fp:
        #     json.dump(final_output_list, fp, indent=4, separators=(',', ': '))
        if len(self.save_json_path) != 0:
            with open(self.save_json_path, 'w') as fp:
                json.dump(final_output_list, fp, indent=4, separators=(',', ': '))
        return results_dict
